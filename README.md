...
- **Метрика Recall для класса `PNoHelmet`:** Текущее значение Recall составляет 0.585. Это означает, что модель пропускает часть людей без шлемов.
  - **Причина:** Сложный фон (деревья, столбы), блики на камере, разнообразие ракурсов и недостаточное количество примеров в обучающей выборке.
- **Ложные срабатывания:** На большом расстоянии возможна путаница с похожими объектами (капюшоны, шапки, велосипедные шлемы).
- **Производительность на CPU:** Обработка видео в высоком разрешении без GPU будет медленной .

---

## Планы по улучшению

- **Расширение датасета:** Добавить больше примеров людей без шлемов с разных ракурсов и в разных условиях освещения (Data Augmentation).
-**Hard Negative Mining:** Дообучить модель на сложных примерах (капюшоны, шапки), чтобы снизить количество ложных срабатываний.
-**Оптимизация инференса:** Экспорт модели в формат TensorRT/ONNX для ускорения обработки видео.
-**Веб-интерфейс:** Разработать полноценный дашборд (React/Vue) для удобного просмотра статистики, видео и управления системой.
- [ ] **Real-time обработка:** Поддержка RTSP-потоков с камер через многопоточность (Celery/Redis Queue).
