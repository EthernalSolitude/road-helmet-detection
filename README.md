...
- **Метрика полноты (Recall) для класса `PNoHelmet`:** 
  ...
  *Причина:* Несбалансированность классов в исходном датасете, сложный фон, блики и разнообразие ракурсов съемки.

- **Ложные срабатывания:** 
  На большом расстоянии или при плохом освещении возможна путаница с визуально похожими объектами (объемные капюшоны, плотные зимние шапки).
...
- **Real-time обработка (RTSP-потоки):**
  Добавление поддержки обработки видеопотоков в реальном времени с IP-камер. Вынесение тяжелого инференса видео в асинхронные фоновые процессы, чтобы обеспечить отказоустойчивость и не блокировать работу основного API-сервера.
